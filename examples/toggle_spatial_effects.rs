...
fn main() {
    App::new()
        ...
        .add_systems(Startup, (startup, set_up_ui).chain())
        .add_systems(
            Update,
            (
...
fn update_source_motion(
    listener: Single<&Transform, (With<SpatialListener3D>, Without<SourceMotion>)>,
    source: Single<(&mut SourceMotion, &mut Transform), Without<SpatialListener3D>>,
    time: Res<Time>,
) {
    ...
    let (mut motion, mut transform) = source.into_inner();
    motion.orbit_angle += core::f32::consts::TAU * time.delta_secs() / orbit_period_seconds;
    motion.bob_phase += core::f32::consts::TAU * time.delta_secs() / bob_period_seconds;
    ...
    transform.translation = listener.translation + offset;
}

fn update_effect_labels(
    effects: Single<&SampleEffects, With<SourceMotion>>,
    ...
    labels.p0().into_inner().0 =
        effect_status(EFFECTS[0].0, EFFECTS[0].1, &followers, effects, 0);
    labels.p1().into_inner().0 =
        effect_status(EFFECTS[1].0, EFFECTS[1].1, &followers, effects, 1);
    labels.p2().into_inner().0 =
        effect_status(EFFECTS[2].0, EFFECTS[2].1, &followers, effects, 2);
}
